...
async function fetchAndProcessUrlContent(url) {
    ...
    try {
        ...
        const selectorsToRemove = 'script, style, link, header, footer, nav, aside, iframe, noscript, .ad, .advertisement, #comments, .sidebar, option, #viewcomment, .toon-nav, .modal, .widget-side-line, .novelticon-modal, #user_donation_coin';
        doc.querySelectorAll(selectorsToRemove).forEach(el => el.remove());
        // ------------------------------------------------------------------
        
        const contentCandidates = Array.from(doc.querySelectorAll('article, main, .post, .entry, .content, #content, #novel_text'));
        let bestText = '';
        let maxTextLength = 0;
